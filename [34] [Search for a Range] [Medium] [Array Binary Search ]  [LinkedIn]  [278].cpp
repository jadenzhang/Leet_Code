...
[34] [Search for a Range] [Medium] [Array Binary Search ] 
[LinkedIn] 
[278].cpp
*/
...
public:
    vector<int> searchRange(vector<int>& nums, int target) {
        ...
        {
            int mid = (low+high)/2;
            if(nums[mid]<target) low = mid+1;//target on right
            else high = mid;//1) equal case: on left or just mid 2) larger case: on left
        }
        ...
        high = nums.size()-1;// no need to change low, because it's starting point
        while(low<high)
        {
            int mid = (low+high+1)/2;//to adjust mid to right, so won't stuck
            if(nums[mid]>target) high = mid-1; //must on left
            else low = mid;// Equal case: right or mid    smaller case: must on right
        }
        res[1] = high;
        return res;
    }
